...
function rampColor(t: number, light = false): string {
...
export function BetweennessDemo({ labels, motionEnabled = true }: { labels: BetweennessDemoLabels; motionEnabled?: boolean }) {
    const { direction, theme } = useLanguage();
    const light = theme === 'light';
    const systemReducedMotion = useReducedMotion();
    const prefersReducedMotion = systemReducedMotion || !motionEnabled;
    ...
    return (
        <div className="w-full" dir={direction}>
            <div className="rounded-2xl border border-white/10 bg-[var(--bg-color)]/60 p-4 md:p-5">
                <svg viewBox="0 0 420 240" className="w-full select-none" role="img" aria-hidden="true">
                            ...
                            return (
                                ...
                                >
                                    <line
                                        ...
                                        stroke={rampColor(t, light)}
                                        strokeWidth={isHover ? 6 : 2 + t * 4}
                                        opacity={onRoute ? 1 : 0.75}
                                        filter={light ? undefined : "url(#science-glow)"}
                    ...
                    {routeD && (
                        <g key={step} pointerEvents="none">
                            ...
                            <circle r={4.2} fill="var(--pop-lime)" style={{ filter: 'drop-shadow(0 0 6px #ccff00)' }}>
                            ...
                            return (
                                <g key={n}>
                                    <circle
                                        ...
                                        fill="var(--bg-color)"
                                        stroke={isEndpoint || onRoute ? 'var(--pop-lime)' : (light ? '#607b8b' : 'rgba(255,255,255,0.45)')}
                    ...
                    <g pointerEvents="none" fontFamily="monospace">
                        <text x={210} y={100} textAnchor="middle" fontSize={11} fill="var(--pop-pink)" fontWeight="bold">
                ...
                {/* live caption */}
                <p className="mt-3 text-xs md:text-sm font-mono min-h-[2.5em]" style={{ color: hoverEdge ? rampColor((hoverCount ?? 0) / MAX_COUNT, light) : 'var(--text-secondary)' }}>
                    {hoverEdge
