...
function centColor(t: number, light = false): string {
    const ramp = light ? [[22,125,164],[111,76,170],[197,40,87]] : RAMP;
...
function buildMetro(key: MetroKey, light = false) {
    ...
    const links: ProjectedLink[] = data.links.map((link, i) => {
        ...
        return { link, d: toPath(link.p), color: centColor(norm, light), norm };
    });
...
export default function CountsMap({ labels }: { labels: CountsMapLabels }) {
    const { direction, theme } = useLanguage();
    ...
    const view = useMemo(() => buildMetro(metro, light), [metro, light]);
                    ...
                    <g>
                        {view.zonePaths.map((d, i) => (
                            <path
                                key={i}
                                d={d}
                                fill={light ? "#e9f2f6" : "rgba(255,255,255,0.015)"}
                                stroke={light ? "#b9d0dc" : "rgba(140,190,240,0.10)"}
                                strokeWidth={1}
                            ...
                            return (
                                ...
                                >
                                    <path
                                        ...
                                        vectorEffect="non-scaling-stroke"
                                        filter={light ? undefined : "url(#counts-glow)"}
                    ...
                    {METRO_ORDER.map(key => {
                        ...
                        return (
                            <button
                                ...
                                style={{
                                    borderColor: selected ? 'var(--pop-pink)' : 'var(--map-border)',
                                    background: selected ? 'rgba(255,0,85,0.10)' : 'var(--map-card)',
                            ...
                            >
                                <span
                                    ...
                                    style={{
                                        background: selected ? 'var(--pop-pink)' : 'var(--map-border)',
                            ...
                            <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: `linear-gradient(${direction === 'rtl' ? 'to left' : 'to right'}, ${light ? "#167da4" : "#00e5ff"}, ${active.color})`,
                ...
                <div className="glass-card rounded-2xl" style={{ padding: '1.25rem 1.5rem' }}>
                    <div className="flex items-center gap-3 font-mono text-[0.65rem] text-white/50">
                        ...
                        <div
                            ...
                            style={{ background: `linear-gradient(${direction === 'rtl' ? 'to left' : 'to right'}, ${light ? "#167da4, #6f4caa, #c52857" : "#00e5ff, #7a5cff, #ff0055"})` }}
                        />
