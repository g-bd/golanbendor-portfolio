...
const DARK_EZOR_COLORS: Record<EzorKey, string> = {
...
export default function CordonMap({ labels, motionEnabled = true }: { labels: CordonMapLabels; motionEnabled?: boolean }) {
    const { language, direction, theme } = useLanguage();
    const light = theme === 'light';
    const EZOR_COLORS: Record<EzorKey, string> = light ? { national: '#167da4', telaviv: '#c52857', jerusalem: '#50853b', haifa: '#7753a6', beersheva: '#ac6821' } : DARK_EZOR_COLORS;
                    ...
                    <g>
                        {BOUND_PATHS.map(b => (
                            <path
                                key={b.id}
                                d={b.d}
                                fill={light ? "#e9f2f6" : "rgba(255,255,255,0.02)"}
                                stroke={light ? (isDim(b.id) ? '#d9e8ee' : '#a9c4d0') : (isDim(b.id) ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.16)')}
                                strokeWidth={1}
                                strokeDasharray="5 5"
                            ...
                            return (
                                ...
                                >
                                    <motion.path
                                        ...
                                        vectorEffect="non-scaling-stroke"
                                        filter={light ? undefined : "url(#cordon-glow)"}
                    ...
                    {EZOR_ORDER.map(key => {
                        ...
                        return (
                            <button
                                ...
                                style={{
                                    borderColor: selected ? color : 'var(--map-border)',
                                    background: selected ? `${color}18` : 'var(--map-card)',
