...
// focus, hidden tab, reduced motion or the explicit pause control. The conference talks
// play inline inside the photo frame (privacy-enhanced YouTube domain, loaded on click);
// while a talk is open the arrows and counter step through the talks instead of the photos.
export default function Events({ t, rtl, motion }: { t: ArchiveContent; rtl: boolean; motion: boolean }) {
    ...
    const running = motion && !paused && !interacting && !focused && talk === null;
    useEffect(() => {
    ...
    const video = talk === null ? null : t.conferences[talk];
    const count = video ? t.conferences.length : t.events.length;
    const change = (delta: number) => {
        if (talk !== null) setTalk((talk + delta + t.conferences.length) % t.conferences.length);
        else setIndex(value => (value + delta + t.events.length) % t.events.length);
    };
    return (
            ...
            <div className={`event-feature ${running ? 'gallery-running' : ''}`}>
                <div className="event-image" onMouseEnter={() => setInteracting(true)} onMouseLeave={() => setInteracting(false)}>
                    {video ? (
                        <>
                            ...
                            <button className="talk-close" onClick={() => setTalk(null)}><X size={13} />{t.backToPhotos}</button>
                        </>
                    ) : (
                ...
                <div className="event-caption">
                    ...
                    <div className="event-controls" onMouseEnter={() => setInteracting(true)} onMouseLeave={() => setInteracting(false)}>
                        <button className="round-control" onClick={() => change(-1)} aria-label={video ? t.previousTalk : t.previous}>{rtl ? <ArrowRight size={18} /> : <ArrowLeft size={18} />}</button>
                        <span className="eyebrow" dir="ltr">{String((talk ?? index) + 1).padStart(2, '0')} / {String(count).padStart(2, '0')}</span>
                        <button className="round-control" onClick={() => change(1)} aria-label={video ? t.nextTalk : t.next}>{rtl ? <ArrowLeft size={18} /> : <ArrowRight size={18} />}</button>
                    ...
                    <button className="text-link conference-link" onClick={() => setTalk(value => value === null ? 0 : null)} aria-pressed={video !== null}>{video ? t.backToPhotos : t.watchConference}{video ? <X size={15} /> : <ArrowUpRight size={17} />}</button>
                </div>
            </div>
            <p className="sr-only">{running ? t.galleryRunning : t.galleryPaused}</p>
            <div className="event-filmstrip">
                {t.events.map((event, i) => (
                    <button className={index === i && !video ? 'selected' : ''} aria-label={`${i + 1}. ${event.title}`} aria-pressed={index === i && !video} key={event.image} onClick={() => { setTalk(null); setIndex(i); }}>
